...
const (
	repoURL    = "https://github.com/velez1337fn/trashtalk-neurons"
	releaseAPI = "https://api.github.com/repos/velez1337fn/trashtalk-neurons/releases/latest"
)
...
func main() {
   ...
   │   │  │                *****
	...
	sideBox := tview.NewFrame(sidePanel)
...
func downloadAndInstall() {
	...
	go func() {
		...
		writeLine(view, "")
		writeLine(view, "> starting download via curl...")
		...
		if assetURL == "" {
			writeLine(view, "ERROR: No release found for arch "+arch)
			writeLine(view, "Please run the GitHub Actions workflow first to build the binary.")
			return
		}

		writeLine(view, "found release! downloading...")

		dlResp, err := httpGet(assetURL)
		if err != nil {
